...
def pytest_addoption(parser):
    ...
    group.addoption(
        ...
        help="only run tests matching integration specified: " "[cassandra,kerberos,mongo,celery,statsd,trino]. ",
    )
    group.addoption(
        "--backend",
        action="store",
...
def dag_maker(request):
    ...
    class DagFactory(LoggingMixin):
        ...
        def __exit__(self, type, value, traceback):
            ...
            if self.want_serialized:
                self.serialized_model = SerializedDagModel(dag, processor_subdir=self.dag_model.processor_subdir)
